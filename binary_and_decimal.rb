...
#    number using the algorithm you devised in class. Print out the decimal val.
def binary_to_decimal
  # binary = Array.new(8)
  binary = Array.new(8).map { rand(2) }
  decimal = 0

  binary.reverse.each_with_index do |digit, i|
    decimal += 2**i if digit == 1
  end
  decimal
end
# 2. Write a method named `array_equals` that accepts two integer arrays as
#    parameters. Method return `true` if the arrays contain the same elements
...
def array_equals(array1, array2)
  longest = [array1.length, array2.length].max
  longest.times do |i|
...
unless array_equals(array1, array2) # both equal case
...
array5 = []
array6 = []
unless array_equals(array5, array6) # equal: empty array case
  puts 'BUG: array_equals should return true.'
  print array5
  print array6
end
